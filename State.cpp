...
//void setY(double newY);
    void State::setY( double newY ){ elements.at(1) = newY; }
...
void State::setDelta( double newDelta ){
    if( newDelta < DELTA_MIN  ){ elements.at(2) = DELTA_MIN; }
    
    else if( newDelta > DELTA_MAX ){ elements.at(2) = DELTA_MAX; }
    
    else { elements.at(2) = newDelta; }
...
//    if (newTheta <= 0 ){ elements.at(3) = 0; }
//    else if ( newTheta > HEADING_MAX ){ elements.at(3) = HEADING_MAX; }
//    else {
//        elements.at(3) = newTheta;
//    }
//   
    
    if ( newTheta < 0 ){  elements.at(3) = HEADING_MAX + newTheta ; }
    else{
        if(newTheta < HEADING_MAX ){
            elements.at(3) = newTheta;
        }
        else{
            elements.at(3) = newTheta - HEADING_MAX;
        }
    }
}
